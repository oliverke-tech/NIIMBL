...
class msc(GeneralTransformer):
    ...
    by accounting for additive and multiplicative effects The Multiplicative Scatter Correction (MSC) is a normalization
    method that attempts to account for additive and multiplicative effects by aligning each spectrum (x_i) with an
    ideal reference one (x_r) as follows:

    .. math:: x_i = m_i x_r + a_{i}x_{i} = m_i x_r + a_i \\text{MSC}(x_i) = a_i - x_{im_{iMSC}}(x_i) = a_i - x_i/m_i
...
class detrend(GeneralTransformer):
    ...
    def fit_transform(self, spc, wave, deg=2):
        ...
        :return detrended_df: spectra array or matrix
        """
...
class derivative(GeneralTransformer):
    """
    Performs n order derivative transformation.
    """
...
class sav_gol(GeneralTransformer):
    """ Performs Savitzky golay smmothing."""
    ...
    def transform(self, spc, window=7, poly=3, deriv=0):
        ...
        self.orig_cols = np.array(pd.to_numeric(spc.columns))
